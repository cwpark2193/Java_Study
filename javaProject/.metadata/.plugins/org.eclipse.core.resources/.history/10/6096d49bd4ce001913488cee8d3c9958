...
public class BoxTest {
	...
	public static void main(String[] args) {
		//1.
		Box<String,Integer> b1 = new Box<String,Integer>("고길동",30);
		String str =b1.getData();
		Integer num = b1.getData2();
		System.out.println(str+" "+num);
	
		String[] str1 = {"홍길동","고길동","구길동"};
		Box.swap(str1, 0, 2);
		...
		MyData[] md = {new MyData("강감찬"),new MyData("을지문덕"),new MyData("이순신")};
		Box.swap(md, 0, 1);
		for(MyData md1 : md ) {
		...
		Box.printArray(str3);
		MyData[] md2 = {new MyData("강감찬"),new MyData("을지문덕"),new MyData("이순신")};
		Box.printArray(md2);
...
//		
//		Box<MyData> b3 = new Box<MyData>(new MyData("싸다김밥"));
